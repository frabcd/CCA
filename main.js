 ...
 * @param {number} winnerNum The player number (1 or 2) who won.
 */
function endGame(winnerNum) {
    // BUG-FIX: This check prevents the function from running multiple times
    if (gameEnded) return;
    gameEnded = true;
    ...
    // BUG-FIX: This is the logic you were missing for the Game Over screen
    $$('winner-text').innerHTML = `${winnerName} WINS!`;

    // --- Add Win Logic ---
    // NEW: Don't record wins against AI
    if (!tutorialState.isGuest && loggedInUser && !gameState.isAIGame) {
        let won = false;
    ...
    // BUG-FIX: Show the Game Over screen after a delay
    setTimeout(() => {
...
// Network state push interval
setInterval(() => {
    // NEW: Don't push state if it's an AI game
    if (!gameState.isOnline || gameState.isAIGame || gameState.get() !== 'ACTIVE' || !online.code) return;
    const me = online.isHost ? players[0] : players[1];
...
function animate() {
    ...
    if (gameState.get() === 'ACTIVE') {
        ...
        if (p1 && p2) {
            // Check for death before updating
            if (!p1.isDead && p1.hp <= 0) p1.die();
            if (!p2.isDead && p2.hp <= 0) p2.die();

            // BUG-FIX: This logic correctly calls endGame
            if (p1.isDead && !gameEnded) {
                endGame(2); // P1 died
            } else if (p2.isDead && !gameEnded) {
                endGame(1); // P2 died
            }

            // Update players if not dead
            // BUG-FIX: Pass specialObjects to the update loop for the AI brain
            if (!p1.isDead) p1.update(delta, p2, specialObjects);
            if (!p2.isDead) p2.update(delta, p1, specialObjects);
        }

        // Projectile update
        for (let i = projectiles.length - 1; i >= 0; i--) {
            if (projectiles[i]) {
                projectiles[i].update(delta, players);
            } else {
                projectiles.splice(i, 1);
            }
        }

        // SpecialObject update
        for (let i = specialObjects.length - 1; i >= 0; i--) {
