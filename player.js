...
export class Player {
    ...
    update(delta, opponent, specialObjects) { // <-- BUG-FIX: Added specialObjects
        ...
        this.velocity.multiplyScalar(1 - 15 * delta); // Friction
    ...
    takeDamage(amount, isDoT = false) {
        ...
        if(!isDoT){
            ...
            // --- DAMAGE FLASH LOGIC (V5.1 BUG-FIX) ---
    ...
    die() {
        ...
        // --- Restore Colors Immediately on Death (V5.1 Bug-Fix) ---
        ...
        // Set state, but let main.js handle the endGame call
        gameState.set('GAME_OVER');
    }

    destroy() {
         // --- Restore Colors Before Destroy (V5.1 Bug-Fix) ---
         if (this.damageFlashTimeout) {
             clearTimeout(this.damageFlashTimeout);
             this.damageFlashTimeout = null;
         }
